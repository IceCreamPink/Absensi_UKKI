...
const Kelas = () => {
  ...
  const filteredKelas = dataKelas.filter(
    (Kelas) =>
      Kelas.kelas?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      Kelas.nama_wali?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const handleDestroy = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`http://127.0.0.1:3000/api/Kelas/${id}`, {
          method: "DELETE",
          // headers: { Authorization: `Bearer ${token}` },
        })
  ...
  const xsml = () => {
    window.location.href = "http://localhost:3000/api/books/exportKelas";
  };
            ...
            {filteredKelas.length > 0 ? (
              filteredKelas.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-16 py-4">{item.kelas}</td>
                  <td className="px-16 py-4">{item.nama_wali}</td>
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      <FontAwesomeIcon icon={faPen} className="pr-4" />
                      SHOW
                    </NavLink>
                  </td>
                  <td className="px-6 py-4">
